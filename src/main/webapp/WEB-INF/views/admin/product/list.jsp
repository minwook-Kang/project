...
<script>
	$(document).ready(function(){
		...
		$("#btn_edit_check").on("click", function(){
			...
			var pro_buy = $("#pro_buy:selected").val();
			
			// 체크 된 상품의 value(pro_num)을 가져옴
		...
		$("#btn_delete_check").on("click", function(){
			...
			if(result){
				...
				$("input[name='check']:checked").each(function(i){
					var pro_num = $(this).val();
					var pro_img = $("input[name='img_"+pro_num+"']").val();
...
<script>
	/* 상품 수정 버튼 클릭 시 */
	var clickEdit = function(pro_num){
		var url = '/admin/product/edit${pm.makeSearch(pm.cri.page)}&pro_num=' + pro_num;
		location.href= url;
	};
</script>
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				...
				<ol class="breadcrumb">
					<li><a href="/admin/product/list"><i class="fa fa-dashboard"></i> 상품 관리</a></li>
					...
					<div class="col-md-12">
						<div class="row text-center">
							...
							<select name="searchType" style="width:180px; height:26px;">
								...
								<option value="name_contents"
									<c:out value="${cri.searchType eq 'name_contents'?'selected':''}"/>>상품명+내용</option>
							...
							<div class="box-body">
								<table class="table table-striped text-center">
									...
									<c:forEach items="${productList}" var="productVO">
										<tr>
											<td><input type="checkbox" name="check" class="check" value="${productVO.pro_num}" style=""></td>
											<td class="col-md-1">${productVO.pro_num}</td>
											<td class="col-md-2">
												<img src="/admin/product/displayFile?fileName=${productVO.pro_img}" style="width:80px;">
											...
											<td class="col-md-2"><a
												href="/admin/product/read${pm.makeSearch(pm.cri.page)}&pro_num=${productVO.pro_num}"
												style="color: black;"> ${productVO.pro_name} </a></td>
											<td class="col-md-1">${productVO.pro_price}</td>
											<td class="col-md-1">${productVO.pro_discount}</td>
											<td class="col-md-2">${productVO.pro_company}</td>
											
											<td><input name="amount_${productVO.pro_num}" type="number" style="width:80px; height:34px; padding-left:5px;" min="1" value="${productVO.pro_amount}" /></td>
											<td>
											<!-- 상품 전시(보임/숨김)기능 -->
											...
											<td class="col-md-2">
												<form class="deleteForm" method="post"
													...
													<input type="hidden" name="pro_img"
														...
														<!-- 수정기능 -->
													<button type="button" name="btn_edit" class="btn btn-default" onclick="clickEdit(${productVO.pro_num});">수정</button>
