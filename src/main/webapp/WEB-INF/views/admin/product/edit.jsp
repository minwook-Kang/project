...
<script>
	$(document).ready(function(){
		...
		CKEDITOR.replace("pro_contents", ckeditor_config);
						...
						<div class="box box-primary">
							...
							<form id='editForm' role="form" action="/admin/product/edit" method="post" enctype="multipart/form-data">
								<div class="box-body">
									...
									<div class="form-group">
										<input name="pro_num" type="hidden" value="${vo.pro_num}" />
										...
										<select class="form-control" id="mainCategory" name="cate_prtcode" style="width:30%; margin-right:10px; display: inline-block;" >
											...
											<c:forEach items="${cateList}" var="list">
												<option value="${list.cate_code}"<c:out value="${vo.cate_prtcode == list.cate_code?'selected':''}"/>>${list.cate_name}</option>
											</c:forEach>
										</select>
									...
									<div class="form-group">
										<label for="exampleInputEmail1">제조사</label> <input
											...
											value="${vo.pro_company}">
									</div>
									<div class="form-group">
										<label for="exampleInputEmail1" style="width:40%; margin-right:10px;">상품 가격</label> 
										<label for="exampleInputEmail1" style="width:40%;">할인 금액</label> 
									...
									<div class="form-group">
										<label for="exampleInputPassword1">상품 내용</label>
										<textarea class="form-control" id="pro_contents" name="pro_contents" rows="8">
											${vo.pro_contents}</textarea>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<label for="exampleInputEmail1" style="width:15%;">판매여부</label><br /> 
										...
										<select class="form-control" id="pro_buy" name="pro_buy" style="width: 15%; display: inline-block;">
										  <option 
										  	<c:out value="${vo.pro_buy == 'Y'?'selected':''}"/>>Y</option>
										  <option
										  	<c:out value="${vo.pro_buy == 'N'?'selected':''}"/>>N</option>
										</select>
									</div>
									<div class="form-group">
										...
										<span class="form-control" style="width:40%; margin-right:10px; display:inline-block;">
											<fmt:formatDate value="${vo.pro_date}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
										<span class="form-control" style="width:40%; display: inline-block;">
											<fmt:formatDate value="${vo.pro_update}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
										<!-- 
										<input type="text" id="pro_update" name="pro_update" class="form-control" 
											value="${vo.pro_update}" readonly />
											 -->
									</div>
								</div>
								...
								<div class="box-footer">
									...
									<button id="btn_list" type="button" class="btn btn-default" >상품 목록으로</button>
								</div>
							</form>
